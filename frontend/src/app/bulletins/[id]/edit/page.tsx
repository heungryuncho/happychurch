...
export default function BulletinEditPage() {
    ...
    useEffect(() => {
        ...
        const fetchBulletin = async () => {
            try {
                ...
                setExistingUrls(response.data.file_urls || []);
            } catch (error) {
                console.error('Failed to fetch bulletin:', error);
                alert('주보 정보를 불러올 수 없습니다.');
                router.push('/bulletins');
            } finally {
                setLoading(false);
            }
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            await apiForm.put(`/bulletins/${params.id}`, formData);
                    ...
                    {existingUrls.length > 0 && files.length === 0 && (
                        <div className="mb-4 p-3 bg-blue-50 border border-blue-100 rounded-xl">
                            <p className="text-sm text-blue-700 font-medium mb-2">
                                기존 파일 {existingUrls.length}개가 있습니다. 새 파일을 선택하면 교체됩니다.
                            </p>
                                ...
                                {existingUrls.map((url, i) => {
                                    ...
                                    return isImage ? (
                                        <img key={i} src={fullUrl} alt={`기존 파일 ${i + 1}`}
                                            className="w-16 h-20 object-cover rounded-lg border border-blue-200" />
                                    ) : (
                                        <div key={i} className="w-16 h-20 rounded-lg border border-blue-200 bg-blue-100 flex items-center justify-center">
